	...
	*/
	...
	public class Files {
		...
		public static String getPath (String str) {
			...
			int dot = str.lastIndexOf (File.separator);
		...
		public static List<File> list (File dir, boolean recurse, boolean folders, List<File> result) {
			...
			while (!stack.isEmpty ()) {
				...
				if (dir.isDirectory ()) {
					...
					if (recurse || level == 1) stack = list (dir, stack);
		...
		public static String mksize (long bytes, String value) {
			return mksize (bytes, value, true);
		}
		
		public static String mksize (long bytes, String value, boolean si) {
			return mksize (bytes, value, si, 3);
		}
		
		public static String mksize (long bytes, String value, boolean si, int num) {
			return mksize (bytes, value, si, num, Locale.getDefault ());
		}
		
		public static String mksize (long bytes, String value, boolean si, int num, Locale locale) {
			return mksize (bytes, value, si, num, locale, new String[] { "", "kb", "Mb", "Gb", "Tb" });
		}
		
		public static String mksize (long bytes, String value, boolean si, int num, Locale locale, String[] lang) {
			...
			if (!value.equals ("")) {
				...
				for (int i = 0; i < Int.size (lang); ++i) {
					...
					if (bytes >= pow) {
						...
						output = format.format (bytes / pow) + " " + lang[i];
		...
		public static long getFolderSize (File dir, long blockSize, CountListener listener) throws IOException {
			...
			while (!stack.isEmpty ()) {
				...
				if (dir.isDirectory ())
					stack = list (dir, stack);
				else
					size += getFileSize (dir, blockSize, listener);
